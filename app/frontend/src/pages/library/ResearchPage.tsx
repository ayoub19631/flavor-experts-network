...
export default function ResearchPage() {
  ...
  usePageMeta({ title: t("research.title"), description: t("research.desc"), path: "/research", locale: lang, hreflang: true });

  useEffect(() => {
    listPublications({ pageSize: 24 }).then((result) => {
      setItems(result.data.filter((item) => item.type !== "book"));
      setLoading(false);
    });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-background">
      <SeoJsonLd data={breadcrumbJsonLd([{ name: t("nav.home"), path: "/" }, { name: t("library.title"), path: "/library" }, { name: t("research.title"), path: "/research" }])} />
      <Navbar />
