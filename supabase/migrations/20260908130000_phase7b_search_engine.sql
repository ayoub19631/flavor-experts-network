-- Phase 7B — unified Postgres search adapter RPCs.
-- SECURITY INVOKER reads only RLS-visible rows. Author hiding uses a boolean DEFINER helper.
SET LOCAL statement_timeout = '30s';

CREATE OR REPLACE FUNCTION public.search_parse_tsquery(p_query text)
RETURNS tsquery
LANGUAGE plpgsql
STABLE
  ...
  WITH hits AS (
    SELECT * FROM (
      ...
      WHERE (cardinality(want) = 0 OR 'people' = ANY (want))
        ...
        AND (
          dd.search_vector @@ q
          OR coalesce(d.search_norm, '') LIKE '%' || nq || '%'
          OR similarity(coalesce(d.search_norm, ''), nq) > 0.28
        )
      ...
      WHERE (cardinality(want) = 0 OR 'suppliers' = ANY (want))
        AND (
          public.normalize_search_text(s.name) LIKE '%' || nq || '%'
          OR similarity(public.normalize_search_text(s.name), nq) > 0.3
        )
      ...
      WHERE (cardinality(want) = 0 OR 'raw_materials' = ANY (want))
        AND (
          m.search_vector @@ q
          OR coalesce(m.search_norm, '') LIKE '%' || nq || '%'
          OR similarity(coalesce(m.search_norm, ''), nq) > 0.28
        )
      ...
      WHERE (cardinality(want) = 0 OR 'jobs' = ANY (want))
        ...
        AND (
          j.search_vector @@ q
          OR public.normalize_search_text(j.title) LIKE '%' || nq || '%'
          OR public.normalize_search_text(coalesce(j.company_name, '')) LIKE '%' || nq || '%'
          OR similarity(public.normalize_search_text(j.title), nq) > 0.28
        )
      ...
      WHERE (
        ...
        AND (
          pub.search_vector @@ q
          OR public.normalize_search_text(pub.title) LIKE '%' || nq || '%'
          OR similarity(public.normalize_search_text(pub.title), nq) > 0.28
          OR EXISTS (
      ...
      WHERE (cardinality(want) = 0 OR 'forum' = ANY (want))
        ...
        AND (
          t.search_vector @@ q
          OR public.normalize_search_text(t.title) LIKE '%' || nq || '%'
          OR similarity(public.normalize_search_text(t.title), nq) > 0.28
        )
      ...
      WHERE (cardinality(want) = 0 OR 'events' = ANY (want))
        ...
        AND (
          e.search_vector @@ q
          OR coalesce(e.search_norm, '') LIKE '%' || nq || '%'
          OR similarity(coalesce(e.search_norm, ''), nq) > 0.28
        )
...
CREATE OR REPLACE FUNCTION public.global_search_suggest(p_query text, p_limit int DEFAULT 8)
RETURNS TABLE (
  entity_type text,
  entity_id text,
  title text,
...
SET search_path = public
AS $$
  SELECT s.entity_type, s.entity_id, s.title, s.href, s.rank
  FROM public.global_search(p_query, NULL, LEAST(GREATEST(coalesce(p_limit, 8), 1), 8), 'relevance') s;
$$;
...
CREATE OR REPLACE FUNCTION public.record_search_event(
  p_query text,
  p_has_results boolean DEFAULT NULL,
...
BEGIN
  IF p_query IS NULL OR char_length(btrim(p_query)) < 2 THEN
    RETURN;
  END IF;
  IF public.search_query_is_sensitive(p_query) THEN
    RETURN;
  END IF;
  INSERT INTO public.search_analytics (user_id, query_hash, query_len, entity_type, has_results, clicked, latency_ms)
  VALUES (
    (SELECT auth.uid()),
    encode(sha256(convert_to(public.normalize_search_text(p_query), 'UTF8')), 'hex'),
    least(char_length(p_query), 200),
    left(coalesce(p_entity_type, ''), 32),
    p_has_results,
    coalesce(p_clicked, false),
    CASE WHEN p_latency_ms IS NULL THEN NULL ELSE least(greatest(p_latency_ms, 0), 60000) END
  );
END;
$$;
...
BEGIN
  IF (SELECT auth.uid()) IS NULL THEN
    RETURN;
  END IF;
  DELETE FROM public.search_recents WHERE user_id = (SELECT auth.uid());
  DELETE FROM public.search_analytics WHERE user_id = (SELECT auth.uid());
END;
$$;
