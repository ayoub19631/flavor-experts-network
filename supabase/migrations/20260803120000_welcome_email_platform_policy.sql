-- Welcome new accounts with a professional bilingual email that includes
-- a concise copy of the platform policy (Terms + Privacy summary).
...
BEGIN
  ...
  html :=
    ...
    || '<div style="display:none;max-height:0;overflow:hidden">Welcome to Flavor Experts Network — your account is ready, plus our platform policy.</div>'
  ...
  plain :=
    ...
    || 'كجزء من تفعيل حسابك، إليك ملخص سياسة المنصة: الالتزام بالشروط، السلوك المهني، حماية البيانات، وخطط العضوية.' || E'\n'
    || 'الشروط: ' || site_url || '/terms' || E'\n'
    || 'الخصوصية: ' || site_url || '/privacy' || E'\n\n'
    || 'Welcome to Flavor Experts Network, ' || safe_name || '.' || E'\n'
    || 'As part of activating your account, here is our platform policy summary and links to the full documents.' || E'\n'
    || site_url || '/terms' || E'\n'
    || site_url || '/privacy' || E'\n'
    || site_url || '/dashboard';
...
COMMENT ON FUNCTION public.notify_welcome_email() IS
  'Sends a bilingual welcome email with platform policy summary when a new user_profiles row is inserted.';
